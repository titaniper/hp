...
class PointServiceTest {
    ...
    fun `charge increases balance and logs history`() {
        ...
        val secondCharge = pointService.charge(1L, 50L)

        assertEquals(150L, secondCharge.point)
        ...
        assertEquals(50L, histories[1].amount)
    ...
    fun `use decreases balance when sufficient`() {
        ...
        val remaining = pointService.use(1L, 80L)

        assertEquals(120L, remaining.point)

        val histories = pointService.history(1L)
        assertEquals(2, histories.size)
        assertEquals(TransactionType.USE, histories.last().type)
        assertEquals(80L, histories.last().amount)
    }

    @Test
    fun `use throws when insufficient balance`() {
        pointService.charge(1L, 50L)
    ...
    fun `history is user specific`() {
        pointService.charge(1L, 40L)
        pointService.use(1L, 10L)
        pointService.charge(2L, 30L)
        ...
        assertEquals(30L, user2History[0].amount)
    }
}
