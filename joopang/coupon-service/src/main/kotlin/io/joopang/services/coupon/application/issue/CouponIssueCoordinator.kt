...
class CouponIssueCoordinator(
    ...
    fun enqueue(command: IssueRequest): CouponIssueQueueResult {
        val requestId = requestIdFor(command.couponTemplateId, command.userId)
        val queueKey = queueKey(command.couponTemplateId)
        val now = Instant.now()

        val addedToQueue = queueOps.addIfAbsent(queueKey, requestId, now.toEpochMilli().toDouble()) ?: false
    ...
    private fun requestIdFor(templateId: Long, userId: Long): String = "$templateId:$userId"
