...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/79b18444-10d9-49cf-85b4-1898202f9f36/Untitled.png)
...
4. **Topic & Partition**
    ...
    ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/dbf132c7-fea8-4dc1-9479-adb08f670f8b/Untitled.png)
    ...
    - `Partition` 에 발행된 순서대로 컨슘함으로써 순차처리를 보장
        ...
        - 동시성 제어의 개념을 생각했을때, 동시에 처리되면 안되는 자원의 Id 등을 메세지의 키로 설정하면 순차처리가 보장되어야 하는 케이스는 보장도 되면서 병렬 처리로 높은 처리량을 보장한 **하이브리드** 전략
            - 포인트 차감
    ...
    - `Producer` 에서 메세지를 발행할 때, 저장될 `Partition` 을 결정하기 위해 메세지의 **키 해시**를 활용하며, 키가 존재하지 않을 경우 균형 제어를 위해 Round-Robin 방식으로 메세지를 기록
        ...
        ```json
        key: "이석범"
        hash: 50649483 // "이석범".hashCode()
        partitionCnt: 3
        
        targetPartition: 1 // (50649483 % 3) + 1
        
        { "3", "userId: 1, couponId : 3"} , {"이석범", "메시지2"} , {"황대선", "메시지3"}
        //"황대선".hashCode() => 54218313 = 0
        ```
...
5. **Consumer Group**
    ...
    - 같은 토픽에 대한 소비주체를 늘리고 싶다면, 별도의 컨슈머 그룹을 만들어 토픽을 구독
        ...
        ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/19e413f1-bb1b-4f32-92f6-1b10ac90ab4e/Untitled.png)
    ...
    - **( 참고 )** 토픽의 Partition 개수와 Consumer 개수에 따른 소비
        ...
        ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/b6f3777a-44b7-4e85-a0b0-3b708ede0291/Untitled.png)
        
        ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/ba2876f1-df33-45fc-86b5-85eb462e17ba/Untitled.png)
        
        ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/74cc946f-e4b3-4f1b-9611-374cd526b410/Untitled.png)
...
8. **Replication**
    ...
    **[ Sequence Diagram ]** 파티션 별의 Leader와 Follower 
    ...
    ![image.png](attachment:d03e160d-5edc-46a0-a4c4-1f381b9c7fef:image.png)
...
단순히 분산 트랜잭션을 처리하는 용도를 넘어서 **도메인간 관심사와 책임을 완벽히 분리**하면서 **장애격리**까지 얻을 수 있는 카프카의 장점을 다시 한번 느껴보길..!
...
- **토픽의 파티션을 활용한 병렬 처리 및 순서 보장 처리**
    ...
    ![image.png](attachment:58ab42bd-3e4e-4dd6-bf12-3721bfbb436f:image.png)
...
Q. **파티션 1,2,3을 컨슈머 1,2가 동시에 가져가지 않나요?**

동시에 처리되면 안되는 메시지는 같은 파티션에 할당하기로 했죠. 

따라서 1번 파티션에 저장된 두 메시지는 동시에 처리되지 못합니다.
...
- 실시간 주문정보(이커머스) & 예약정보(콘서트)를 카프카 메시지로 발행하도록 변경합니다.
    ...
    - mockAPI를 호출하던 로직을 카프카 메시지 발행으로 전환합니다.
...
비동기 메시지 발행을 메인 트랜잭션 **외부**에서 처리하는 이유는?
...
2. 카프카의 발행 이후에 트랜잭션의 커밋에 실패하면 존재하지 않는 데이터의 이벤트를 발행한 것이기 때문에.. (주워담을 수 없는 물을 흘린…)
    1. 333번 주문이 이벤트로 흘러나갔으나 주문 테이블에 333 ID를 갖는 주문은 없었다……

다만, 외부에서 처리를 하면 발생하는 문제는 `DB는 성공했지만 Kafka 전송이 실패하는 케이스`  를 만들고 싶지 않은 것입니다. 이를 처리하기 위한 패턴이 **Transactional Outbox Pattern** 입니다.

카프카에 적재를 원천테이블과 동일하게 적재해야하는 것은 프로듀서의 책임이고 이후 발행된 메시지를 처리하는 것은 컨슈머의 책임으로 분리해서 생각해야 합니다.

![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/2413fef4-0ff6-4d98-b298-a9aae2de1a1f/Untitled.png)
...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/15723a77-16a4-487a-9c24-da58ae1a7292/Untitled.png)

발행자는 우리의 메시지가 유실되지 않았다는 사실을 증명하기 위해 할 수 있는 방법은?

카프카를 직접 확인해서 메시지가 전부 발행되었는지 확인하여 검증하는 방법
...
// 주문 이벤트 발행
Application Event Listener(After_Commit) {
	아웃박스 데이터 Kafka 에 전송
}
...
- **Publisher**
    ...
    > 비즈니스 로직 수행과 Outbox 저장을 **동일한** 트랜잭션 내에서 수행함으로써 비즈니스 로직 성공 = 이벤트 발행을 항상 보장함
    > 
- **Consumer 1**
    - Outbox 변경
...
orderCreatedEventRepository.findAllByStatus(INIT).forEach {
	if(it.createdAt < now() - 5minutes) {
		kafkaPublisher.publishOrderInfo(); // 발행에 실패한 케이스에 대한 발행
...
```
대기업에서 카프카(스트림) 전담팀조차 카프카 운영이 힘들어서 장애가 잦았다고 들은 적 있습니다.
특히 "연결 단계의 장애"를 카프카 성숙도가 낮은 개발자들이 잘 대응하지 못한다고 들었습니다.
---

Q1. 대용량 트래픽에서 요청이 몰려 컨슈머의 처리지연이 생겨버린다면
카프카 코디네이터는 리밸런싱으로 컨슈머를 out 시켜버린다고 이해하였습니다.
그러면 안그래도 서버가 숨지는 트래픽이 몰려오고 있는데, 처리량을 점점 더 감당 못하는 컨슈머들이 연쇄적으로 터져버리는 것 아닐지요?
컨슈머가 reconnect 시그널을 보내서 다시 살아나려 해도, 살아날 때 리밸런싱으로 인한 레이턴시가 또 생겨버려서 병목이 증가할 것 같습니다.
이런 장애사례를 겪어보신 적 있으신지, 어떻게 해결하셨는지 궁금합니다!

카프카 컨슈머로 처리하는 것의 장점.
실시간의 트래픽을 고스란히 부하로 받지 않아요.
우리의 컨슈머는 처리할 수 있는 메시지만큼만 순차적으로 받아서 처리해요.
트래픽이 몰린다고 컨슈머에 부하가 직접적으로 늘지는 않아요. 단, 처리량은 떨어지겠죠.
메시지에 처리되지 못한 메시지들이 쌓여있을 수는 있죠.

Q2. 저희 이커머스는 **모놀리식으로 구성**되어있는데 이건 카프카에 적합한 구성이 아닐 것 같습니다.
실제 처리부인 컨슈머에서 부하 또는 장애가 자주 발생할것같은데, 컨슈머 장애 때문에 모놀리식 서버가 뻗어서 퍼블리시조차 못하면 문제가 될 것 같습니다. 
따라서 실무에서 실제 서버 구성시에는 컨슈머는 별도 서버로 둘 것 같은데 맞나요? 실제 사용사례로 WAS서버와 컨슈머서버가 구체적으로 몇개 스케일아웃 / 컨슈머 몇개로 분리해서 쓰셨을지 궁금합니다.
+ 쿠폰 퍼블리시 코드와, 쿠폰 이슈 컨슘 코드는 코드베이스(깃 레포) 조차 별도로 분리해서 쓰는지 궁금합니다.

당연히 API 서버와 컨슈머 서버는 별도의 노드/파드로 분리해서 운영해요.
API 서버는 실시간 트래픽에 의해 auto-scailing이 활성화가 잘 되는데, 
컨슈머는 메시지를 순차적으로 처리하기 때문에 큰 부하를 실질적으로 밀어넣지는 못해요. 그래서 인스턴스 개수를
유동적으로 전환을 많이 하지는 않습니다.
...
**API 서버** presentation -> application

**컨슈머 서버** consumer -> application

우리는 모놀리식으로 구성되어 있더라도 카프카를 사용하면 좋을법한 상황 2.

- 쿠폰발급처럼 순차적으로 쿠폰을 발급시키기도 하면서 DB 부하를 줄일때..
- 엑셀업로드나 벌크처리하는 프로세스....
  - 엑셀파일 업로드해서 API 서비스에서 열심히 처리하더라도.. latency 잘 안나오죠. API 서버의 직접적인 스레드 부하.
  - 엑셀파일을 업로드를 해놓고, 처리를 컨슈머에서 처리하게하면?
    - 엑셀파일 -> AWS S3 같은데 올려놓고, 카프카 토픽에 이 s3 파일 처리해~~
    - 컨슈머가 처리하면 될 것 같아요.
    오래걸리는 작업도 컨슈머가 처리하면 부하로 인식되지는 않죠. 다만 처리가 오래걸려서 리밸런싱 같은게 일어나지 않도록 설정을 조정하긴 해야겠지만요.

Q3. 케이스 바이 케이스겠지만, 파티션 수 / 컨슈머 수는 보통 몇개로 잡고  n:m 비율로 나눠 쓰시나요? 실무에서 정책 정하는 기준이 있으실까요?

default값은 파티션수 -> 3개 (파티션의 삭제는 불가능해요. 추가는 가능해요(쉽지는 않지만..))
파티션의 삭제가 불가한 이유는? 메시지가 파티션에 저장되어있기 때문이죠.
추가는 가능하지만 쉽지 않다고 말씀드린 이유는? 파티션 수가 늘어나면 같은 키가 할당되는 파티션이 달라지죠.
순차보장이 틀어질 수 있어요.
...
```
오늘 설명해주신 것들 중 Cluster랑 Replication 부분 다시 한번 만 더 설명해주실 수 있나요?
=> 다음주 화요일 9시에 진행할게요!
```

```
step17번에 mockAPI를 호출하던 로직을 카프카 메시지 발행으로 전환합니다. 라고 작성해주셨는데 mockAPI가 자바 테스트 할 때 의 그 mockAPI를 지칭하는것이 맞을까요?

그렇다면 이 과제의 내용은 테스트로 작성된 mockAPI를 카프카로 변경하라는게 맞는지 궁금합니다
