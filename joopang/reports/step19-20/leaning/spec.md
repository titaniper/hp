...
![image.png](attachment:b661b519-48d9-45b1-82f4-da35567ccfa0:image.png)

![image.png](attachment:17f77fdc-b5c2-462f-a013-8ce96383cef3:image.png)
...
![image.png](attachment:5e5ac5ba-5042-43a3-accc-7dd2f298624c:image.png)

- 메트릭 모니터링(Metric Monitoring)
    - 메트릭 모니터링은 시스템이나 애플리케이션이 노출하는 CPU 사용률, 메모리 사용량 등과 같은 메트릭을 모니터링하여 시스템의 강 상태와 성능을 추적하는 시스템
    ...
    ![image.png](attachment:97212694-0b52-465e-893e-9b26b517f59b:image.png)
...
- **고가용성 ( High Availability )**
    ...
    - 99%의 가용성: 100일중에 1일은 장애가 난다, 1년기준: 3.5일
        ...
        - 99.999999% ([aws의 SLA](https://aws.amazon.com/ko/legal/service-level-agreements/))
            ...
            ![image.png](attachment:11c5a6a6-0ce0-4d47-9378-1ff3eddd863d:image.png)
    ...
    ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/23461316-6721-4813-8158-ec76b5c25690/Untitled.png)
    
    1. DB 리소스 부족 문제로 인해 DB 서버에 연결이 불가능한 경우
    
    ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/a27003d9-f4b2-419a-97f2-733a10828771/Untitled.png)
...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/442e36cc-f1f4-4c26-8954-09f74e7de88d/Untitled.png)
...
![image.png](https://prod-files-secure.s3.us-west-2.amazonaws.com/83c75a39-3aba-4ba4-a792-7aefe4b07895/f21b8803-62e9-4129-9937-3643a3042ab4/image.png)
...
```kotlin
NASA는 뭘 실패했다고 해서, 그 담당자를 탓하고 자르는 방식으로 문제를 해결하지 않습니다.
잘라서 문제를 덮기 전에, 기본적으로 이렇게 생각할 줄 아는 거죠.
"실패한 사람이 그 문제에 대해 가장 잘 아는 사람이다."

뭘 하면 실패하는지 아는, 경험치가 가장 많은 사람이라고 생각하기 때문이죠.
그래서 NASA는 실패한 사람에게 이렇게 말합니다.
"필요하다면 모든 지원을 줄 수 있다. 하지만 네가 직접 이 문제를 해결해라."
```

물론 NASA도 처음부터 실패를 발판 삼는 문화를 정착했던 것은 아닙니다. 초창기 NASA는 여느 다른 기업과 크게 다르지 않았습니다. 그러나 `챌린저 우주 왕복선 폭발 사고`로 인해, NASA의 기업 문화는 완전히 뒤바뀌게 되었습니다.

`챌린저 우주 왕복선 폭발 사고` 는  발사 73초 만에 공중에서 폭발해 탑승중이던 승무원 7명이 전원 사망하는 참사였습니다. 안타깝게도 관리자들 몇몇은 기계적 결함과 사고 가능성에 대해 예측했었지만, 커뮤니케이션 실패로 결국 발사가 진행되었고, 참사가 발생했다고 합니다.

이후 NASA는 모든 미션에서 철저한 검증과 모든 변수를 계산해 최적의 상태가 기다려 주기로 했고, 무언가 실패했을 때 가장 큰 손실은 사람을 잃고, 경험을 잃는 것임을 깨달았다고 합니다.

우리의 업무 환경 역시 마찬가집니다. 사람은 실패를 통해 배우기 때문에, 실패했을 때 책임을 묻고 비난만 해서는 더 나아질 수 없습니다. 실패했을 때 원인을 분석하여 다시 같은 문제가 벌어졌을 때를 대비해야 합니다.

문제를 일으킨 코드를 만든 사람을 색출하고, 그에게 책임을 지우면? → 아무도 도전적인 업무를 하지 않습니다. 커뮤니케이션이 완전히 경색됩니다. 이런 조직은 백날 장애대응 훈련을 해도 무용지물입니다.

그러니 여러분이 만약 어디선가 장애 대응을 해야한다면! 팀원들의 실패를 용인하고, 비난하기 보다는 프로세스를 통해 문제를 해결하고, 조직 내에 심리적인 안정감을 끌어올리는 자세가 필요합니다.
