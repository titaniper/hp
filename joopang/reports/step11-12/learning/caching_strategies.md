...
* **락 획득 대기 시간(Lock Wait Time)**: `tryLock(waitTime, leaseTime, unit)`에서 첫 번째 파라미터로 설정하는 값으로, **스레드가 락을 얻기 위해 얼마 동안 기다릴지**를 의미합니다. 너무 길면 요청 스레드가 오래 블로킹되고, 너무 짧으면 대표 스레드가 락을 잡고 계산을 끝내기도 전에 포기하고 DB를 직접 찌르는 요청이 늘어납니다.
  * **예시**: 인기 상품 캐시 키 하나를 1초 안에 계산할 수 있다면 `waitTime=2초`로 잡아 1개 스레드가 계산하는 동안 다른 스레드는 최대 2초까지만 기다리게 합니다. 2초 내에 락을 못 잡으면 짧게 슬립 한 뒤 캐시를 다시 조회해 이미 채워졌는지 확인합니다.
* **락 점유 시간(Lock Hold/Lease Time)**: 두 번째 파라미터로 설정하며, **대표 스레드가 얼마나 오래 락을 보유할 수 있는지**를 제한합니다. 계산이 끝났는데도 락을 해제하지 않거나 장애로 해제 되지 않는 경우를 대비한 안전장치입니다.
  * **예시**: 인기 상품 쿼리가 최대 4초가 걸린다면 `leaseTime=5초` 정도로 설정해, 대표 스레드가 5초 안에 캐시를 채우지 못하면 락이 자동 해제되도록 합니다. 너무 짧게 잡으면 계산 도중 락이 풀려 다른 인스턴스가 중복 계산을 하게 되므로, **평균 처리 시간 + 여유 버퍼(20~30%)**를 권장합니다.
* **추천 값 설정**:
  1. **처리 시간 기준**: 실제 DB 조회 + 캐시 적재 시간이 500ms라면 `waitTime`은 2~3배(1~1.5초), `leaseTime`은 4~5배(2~3초)로 시작합니다.
  2. **모니터링 피드백**: 락 대기 타임아웃이 자주 발생하면 wait/lease를 늘리거나 워밍업·Soft TTL을 병행합니다. 반대로 대기 스레드가 응답 SLA를 넘기면 waitTime을 줄이고 실패 시 즉시 캐시 재조회나 fallback 값을 반환하도록 조정합니다.
  3. **핫키 전용 정책**: 초핫키는 별도 설정으로 wait/lease를 더 길게 주고, 비핫키는 공용 짧은 값을 사용해 전체 스레드 풀이 묶이지 않게 합니다.
