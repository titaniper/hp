...
- 상품별 락 대기시간(2초)이 지나면 곧바로 실패시키므로, 초고부하 상황에서는 사용자 재시도가 늘어날 수 있다. 필요 시 상품 유형에 따라 대기시간을 조정하거나 큐 기반 선착순 정책을 고려해야 한다. 재시도가 폭주하는 구간에는 Redis Stream/BlockingQueue로 주문 요청을 먼저 큐잉하고, 소비자가 순서대로 락을 획득하도록 하여 우선순위를 보장할 계획이다.
