...
- 템플릿 단위 단일 락은 공정성을 보장하지 않는다. 특정 사용자가 반복적으로 락을 선점하면 나머지 사용자가 장시간 대기할 수 있으며, 필요 시 Redisson `tryLock` 대신 `tryLockAsync` + 큐잉/레이트리밋를 고려해야 한다. 특히 선착순 이벤트에서는 Redis Stream/BlockingQueue로 요청을 직렬화한 뒤 큐 소비자가 순서대로 락을 잡도록 하는 보조 대기열을 추가해 우선순위를 제어할 계획이다.
