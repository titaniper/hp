...
| `DECR / DECRBY` | 값을 정수로 해석하여 감소시킵니다. (재고 차감 등) | O(1) |
...
| `ZINTERSTORE dest numkeys key...` | 여러 집합의 교집합을 구하고 점수를 합산/최대/최소 처리하여 저장합니다. | O(N*K) |
| `ZPOPMIN / ZPOPMAX` | 점수가 가장 낮은/높은 요소를 꺼내고 삭제합니다. (대기열 처리) | O(log N) |
| `ZREMRANGEBYSCORE key min max` | 점수 범위에 해당하는 요소를 삭제합니다. (오래된 데이터 정리) | O(log N + M) |
...
판매량(70%)과 매출액(30%)을 반영한 종합 랭킹을 만듭니다.

```bash
# 판매량 랭킹 (sales)
> ZADD rank:sales 100 "product:A" 50 "product:B"
# 매출액 랭킹 (revenue)
> ZADD rank:revenue 10000 "product:A" 500000 "product:B"

# 두 랭킹을 합치되 가중치 적용 (sales*7, revenue*3) 하여 rank:total에 저장
> ZINTERSTORE rank:total 2 rank:sales rank:revenue WEIGHTS 7 3
(integer) 2

# 결과 확인
> ZRANGE rank:total 0 -1 WITHSCORES
1) "product:A"
2) "30700"  # (100*7) + (10000*3)
3) "product:B"
4) "1500350" # (50*7) + (500000*3)
```

#### D. 슬라이딩 윈도우 (오래된 데이터 삭제)

최근 1시간 데이터만 유지하고 싶을 때, Score를 타임스탬프로 사용합니다.

```bash
# 현재 시간: 1700003600
# 1시간 전(1700000000) 이전의 데이터 삭제
> ZREMRANGEBYSCORE event:log -inf 1700000000
(integer) 5
```
