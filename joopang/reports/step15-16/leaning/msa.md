...
- 하나의 큰 애플리케이션을 **작고 독립적인 여러 개의 서비스**로 나누어 개발, 배포, 운영하는 아키텍처 스타일.
- 각 서비스는 **독립적인 프로세스**에서 실행되며, HTTP API나 메시지 큐 같은 가벼운 메커니즘으로 통신한다.
- 서비스별로 **자체 데이터베이스**를 가질 수 있으며(Polyglot Persistence), 서로 다른 프로그래밍 언어나 기술 스택을 사용할 수 있다.

## 2. 특징

- **서비스의 자율성**: 각 서비스는 독립적으로 배포 및 확장이 가능하다.
- **조직 구조 반영**: 콘웨이의 법칙(Conway's Law)에 따라, 기술 계층(UI, DB 등)이 아닌 비즈니스 역량(주문, 결제 등) 중심으로 팀과 서비스가 구성된다.
- **장애 격리 (Fault Isolation)**: 하나의 서비스 장애가 전체 시스템의 장애로 전파되는 것을 막을 수 있다.
- **분산 데이터 관리**: 데이터의 강한 일관성보다는 결과적 일관성(Eventual Consistency)을 지향한다.

## 3. 모놀리식(Monolithic) vs MSA

| 구분 | 모놀리식 (Monolithic) | 마이크로서비스 (MSA) |
| :--- | :--- | :--- |
| **구조** | 단일 코드베이스, 단일 프로세스 | 여러 개의 작은 서비스 집합 |
| **배포** | 전체 애플리케이션 재배포 필요 | 변경된 서비스만 독립 배포 가능 |
| **확장성** | 전체 인스턴스 복제 (Scale-out) | 특정 서비스만 선별적 확장 가능 |
| **복잡도** | 초기 개발 용이, 규모 커지면 유지보수 어려움 | 초기 구성 복잡, 규모 커져도 관리 용이 |
| **기술 스택** | 단일 기술 스택에 종속 | 서비스별 최적의 기술 선택 가능 |

## 4. 장점

- **빠른 배포 주기**: 변경 사항이 있는 서비스만 배포하면 되므로 CI/CD 속도가 빨라진다.
- **유연한 확장성**: 트래픽이 몰리는 서비스(예: 주문)만 자원을 늘릴 수 있어 효율적이다.
- **기술 도입 용이**: 새로운 기술을 특정 서비스에만 시범적으로 도입해볼 수 있다.

## 5. 단점 및 도전 과제

- **분산 시스템의 복잡성**: 네트워크 지연, 트랜잭션 관리, 장애 추적 등이 어렵다.
- **운영 비용 증가**: 관리해야 할 서비스와 인프라가 많아져 DevOps 역량이 필수적이다.
- **테스트 어려움**: 여러 서비스가 연동되는 통합 테스트(E2E) 환경 구성이 까다롭다.

## 6. 주요 패턴 및 기술

- **API Gateway**: 클라이언트의 요청을 받아 적절한 서비스로 라우팅하고, 인증/인가, 로깅 등을 통합 처리.
- **Service Discovery**: 동적으로 변경되는 서비스의 IP와 포트를 찾아주는 메커니즘 (예: Eureka, Consul).
- **Circuit Breaker**: 장애가 발생한 서비스로의 요청을 차단하여 전체 시스템 보호 (예: Resilience4j).
- **Event Driven Architecture**: 서비스 간 결합도를 낮추기 위해 비동기 메시징(Kafka, RabbitMQ) 사용.
