...
## 1. 개요
- 기존 모놀리식(Spring Boot 단일 애플리케이션)을 `common`, `order-service`, `coupon-service` 3개 Gradle 모듈로 분리했습니다.
...
## 6. 향후 보완 과제
1. **API Gateway & 라우팅**: 클라이언트 → 서비스 간 호출을 API Gateway를 통해 일원화하고, 인증/트래픽 관리 정책을 분리합니다.
